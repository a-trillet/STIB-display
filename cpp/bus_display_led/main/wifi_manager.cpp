...
WiFiManager::WiFiManager() 
    : initialized_(false), ap_mode_active_(false), sta_connected_(false),
      current_ssid_(""), connection_status_("Not connected"), wifi_event_group_(nullptr) {
}
...
bool WiFiManager::connect_sta(const std::string& ssid, const std::string& password) {
    if (!initialized_) {
        ESP_LOGE(TAG, "WiFi manager not initialized");
        return false;
    }
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED: {
                ...
                wifi_mgr->connection_status_ = "Failed to connect to " + wifi_mgr->current_ssid_;
